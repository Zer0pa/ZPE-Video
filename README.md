...
- We do not claim C2PA conformance. The manifest binding is a hash-reference shape for receipt bytes, not a passed C2PA conformance result.
...
| Field | Value |
|-------|-------|
| Release posture | Staged v0.1.0 receipt surface; not a portfolio verdict |
| Receipt-core verdict | PASS (per authority packet) |
| Authority bundle | regenerated by `scripts/authority_bundle.py` per release; canonical hash lives in [`current_authority_packet.json`](proofs/manifests/current_authority_packet.json) |
